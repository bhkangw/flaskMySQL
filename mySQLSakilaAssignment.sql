...
JOIN city ON address.address_id = city.country_id
...
SELECT store.store_id, city_id, customer.first_name, customer.last_name, customer.email, address.address
from customer
JOIN address ON address.address_id = customer.customer_id
JOIN store ON customer.store_id = store.address_id
...
WHERE actor_id = 5 AND rating = "G" AND film.special_features LIKE "%Behind the Scenes%"
